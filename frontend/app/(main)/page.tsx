...
const categoriesIconMap: Record<
  string,
...
const baiViet = [
  {
    ...
    tom_tat:
      "Chip A19 Bionic mạnh mẽ, camera cải tiến đáng kể, nhưng giá vẫn là rào cản lớn với nhiều người.",
    ngay: "20/05/2025",
    hinhAnh:
      "https://images.unsplash.com/photo-1695048133142-1a20484d2569?w=800&q=80&fit=crop",
  },
  {
    id: 2,
    tag: "So sánh",
    ...
    tom_tat:
      "Hai chiếc laptop cao cấp với điểm mạnh khác nhau — Apple hay Windows sẽ phù hợp với bạn hơn?",
    ngay: "17/05/2025",
    hinhAnh:
  ...
  },
  {
    ...
    tom_tat:
      "Thế hệ mới của dòng gập đến gần — Samsung hứa hẹn màn hình bền hơn và pin lâu hơn.",
    ngay: "14/05/2025",
    hinhAnh:
      "https://images.unsplash.com/photo-1610945415295-d9bbf067e59c?w=800&q=80&fit=crop",
  },
  {
    id: 4,
    tag: "Đánh giá",
    ...
    tom_tat:
      "Dòng đồng hồ mới nhất của Samsung có đáng để nâng cấp hay không? Đánh giá nhanh tính năng, pin và thiết kế.",
    ngay: "11/05/2025",
    hinhAnh:
      "https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=800&q=80&fit=crop",
  },
...
const baiVietMini = [
  ...
  },
  {
    ...
    hinhAnh:
      "https://images.unsplash.com/photo-1517519014922-8fc7eea9b7f1?w=800&q=80&fit=crop",
  },
  {
    id: 6,
    tag: "Xu hướng",
  ...
  return (
    <Link href={`/sanpham/${p.slug}`}>
      <div className="group bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 flex flex-col relative cursor-pointer h-full">
        {p.badge && (
          <span
            className={`absolute top-3 left-3 z-10 text-white text-[10px] font-bold px-2.5 py-1 rounded-full ${badgeColor[p.badge] ?? "bg-gray-500"}`}
          >
            {p.badge}
          </span>
        )}
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      >
        ...
        <div
          className={`relative z-10 max-w-screen-xl mx-auto px-6 py-16 md:py-24 transition-all duration-300 ${isAnimating ? "opacity-0 translate-y-2" : "opacity-100 translate-y-0"}`}
        >
          <span className="inline-block text-xs font-semibold text-red-400 uppercase tracking-widest mb-3">
            {b.tag}
          </span>
          <h1 className="text-4xl md:text-6xl font-bold text-white leading-tight whitespace-pre-line mb-4">
            {b.title}
          </h1>
          <p className="text-gray-300 text-sm md:text-base max-w-lg mb-7">
            {b.sub}
          </p>
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-gray-300 bg-white/10 px-4 py-3 rounded-xl border border-white/10">
              {b.badge}
            </span>
          </div>
        </div>
        <button
          ...
          className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center text-white transition-all"
        >
          <ChevronLeft className="w-4 h-4" />
        </button>
        <button
          onClick={() => goTo(current + 1)}
          className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center text-white transition-all"
        >
        ...
        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-2 z-20">
          {banners.map((_, i) => (
            <button
              key={i}
              onClick={() => goTo(i)}
              className={`h-1.5 rounded-full transition-all duration-300 ${i === current ? "w-8 bg-red-500" : "w-2 bg-white/30"}`}
            />
          ))}
        </div>
      </section>
      ...
      <section className="max-w-screen-xl mx-auto px-6 -mt-5 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {tienIch.map((t) => (
            <div
              key={t.title}
              className="flex items-center gap-3 bg-white border border-gray-100 rounded-xl p-3.5 shadow-sm"
            >
        ...
        <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide pb-1">
          {loadingCats ? (
            Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="flex-shrink-0 h-11 w-28 rounded-xl bg-white border border-gray-100 animate-pulse"
              />
            ))
          ) : errorCats ? (
            ...
            categories
              .filter(
                (cat) =>
                  cat.slug !== "phu-kien" && cat.category_name !== "Phụ kiện",
              )
              .map((cat) => {
                ...
                return (
                  <Link
                    ...
                    className="flex-shrink-0 flex items-center gap-2 bg-white border border-gray-100 rounded-xl px-4 py-2.5 hover:border-red-200 hover:bg-red-50 transition-all group"
                  >
                    <Icon className="w-4 h-4 text-gray-500 group-hover:text-red-500 transition-colors" />
                    <span className="text-sm font-medium text-gray-600 group-hover:text-red-600 whitespace-nowrap">
                      {cat.category_name}
                    </span>
                  </Link>
                );
          ...
          <Link
            href="/sanpham"
            className="flex-shrink-0 flex items-center gap-1.5 text-sm font-medium text-red-600 hover:text-red-700 px-3 whitespace-nowrap"
          >
                    ...
                    <Link key={p.id} href={`/sanpham/${p.slug}`}>
                      <div className="group overflow-hidden rounded-[28px] border border-gray-100 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl h-full">
                        <div className="relative overflow-hidden bg-slate-50">
                          ...
                          />
                          <div
                            className={`absolute top-3 left-3 z-20 w-9 h-9 flex items-center justify-center text-sm font-black rounded-br-xl rounded-tl-2xl
                            ${
                              i === 0
      ...
      <section className="max-w-screen-xl mx-auto px-6 mt-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Link
            href="/sanpham?khuyen-mai"
            className="group block overflow-hidden rounded-2xl bg-white border border-gray-200 shadow-sm hover:shadow-md transition-all"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-5 items-center">
              <div>
                <span className="inline-block bg-red-50 text-red-600 text-[10px] font-semibold uppercase tracking-[0.3em] px-3 py-1 rounded-full mb-3">
                  Sale giữa tháng
                </span>
                ...
                <p className="text-sm text-gray-600 mt-3">
                  Nhiều sản phẩm giảm giá sốc, mua ngay kẻo lỡ.
                </p>
              ...
              <div className="overflow-hidden rounded-2xl bg-gray-100">
                <img
                  src="https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800&q=80&fit=crop"
                  alt="sale"
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-5 items-center">
              <div>
                <span className="inline-block bg-slate-100 text-slate-700 text-[10px] font-semibold uppercase tracking-[0.3em] px-3 py-1 rounded-full mb-3">
                  Gaming
                </span>
                <h3 className="text-2xl font-bold text-gray-900 leading-snug">
                  Chơi không ngắt
                ...
                <p className="text-sm text-gray-600 mt-3">
                  Máy gaming hiệu năng cao, pin bền lâu cho cả ngày.
                </p>
                <p className="text-sm text-red-600 font-semibold mt-4">
                  Pin trâu, hiệu năng đỉnh
                </p>
              </div>
              <div className="overflow-hidden rounded-2xl bg-gray-100">
                <img
      ...
      <section className="max-w-screen-xl mx-auto px-6 mt-12 mb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Link
            href={`/tin-tuc/${baiViet[0].id}`}
            className="group col-span-1 lg:col-span-2 bg-white border border-gray-100 rounded-[28px] overflow-hidden hover:shadow-xl transition-all duration-300"
          >
            ...
            {baiViet.slice(1).map((bv) => (
              ...
              >
                ...
                <div className="flex-1 p-4">
                  <span className="text-[10px] font-semibold uppercase tracking-[1px] text-blue-600">
                    {bv.tag}
                  </span>
                  <h4 className="mt-2 text-sm font-semibold text-gray-900 leading-snug line-clamp-2 group-hover:text-red-600 transition-colors">
                    {bv.tieu_de}
                  </h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          {baiVietMini.map((item) => (
            ...
            >
              ...
              <div className="p-4">
                <span className="text-[10px] font-semibold uppercase tracking-[1px] text-red-500">
                  {item.tag}
                </span>
                <h4 className="mt-2 text-sm font-semibold text-gray-900 leading-snug line-clamp-2">
                  {item.tieu_de}
                </h4>
              </div>
            </Link>
