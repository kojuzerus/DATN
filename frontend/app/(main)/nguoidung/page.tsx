...
export default function NguoiDungPage() {
  ...
  const loadDistricts = useCallback(
    async (provCode: number): Promise<SelectOption[]> => {
      setLoadingDist(true);
      setDistricts([]);
      setWards([]);
      try {
        ...
        const d = await r.json();
        const list = (d.districts || []).map((x: any) => ({
          code: x.code,
          name: x.name,
        }));
        setDistricts(list);
  ...
  const loadWards = useCallback(
    async (distCode: number): Promise<SelectOption[]> => {
      setLoadingWard(true);
      setWards([]);
      try {
        const r = await fetch(`${GEO_API}/d/${distCode}?depth=2`);
        const d = await r.json();
        const list = (d.wards || []).map((x: any) => ({
          code: x.code,
          name: x.name,
        }));
        setWards(list);
  ...
  const handleSaveAddress = async () => {
    const { receiverName, phone, province, district, ward, detailAddress } =
      form;
    if (
      !receiverName ||
      !phone ||
    ...
    try {
      const url = editingAddr
        ? `${API_URL}/api/addresses/${editingAddr._id}`
      ...
      setFormErr(e.message || "Có lỗi xảy ra");
    } finally {
      setFormSaving(false);
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setSaveErr(e.message || "Có lỗi xảy ra");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPwErr(e.message || "Có lỗi xảy ra");
    } finally {
      setPwSaving(false);
    }
  };
  ...
  const accountType = user.googleId
    ? "Google"
    : user.zaloId
      ? "Zalo"
      : "Tài khoản thường";
        ...
        <aside className="w-full md:w-64 shrink-0">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="bg-gradient-to-br from-red-500 to-red-700 px-5 py-6 text-center">
              ...
              <p className="text-white font-semibold mt-3 text-base leading-tight">
                {user.hoTen}
              </p>
              <p className="text-red-200 text-xs mt-1">
                {user.soDienThoai || user.email || ""}
              </p>
              {user.role === "admin" && (
            ...
            <nav className="p-2">
              ...
              {user.role === "admin" && (
                <Link
                  href="/admin"
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-amber-500 hover:bg-amber-50 transition-all"
                >
                ...
                {!editing ? (
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={saving}
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 disabled:opacity-60 text-white text-sm font-medium transition-colors"
                    >
                ...
                {[
                  {
                    ...
                    edit: (
                      <input
                        ...
                        className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-100 transition"
                  ...
                  (
                    { icon: Icon, bg, ic, label, editable, view, edit },
                    i,
                    arr,
                  ) => (
                    <div key={label}>
                      ...
                      {i < arr.length - 1 && (
                        <div className="border-t border-dashed border-gray-100 mt-5" />
                      )}
                    </div>
                  ),
                )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="sm:col-span-2 sm:w-1/2">
                      ...
                      <SearchableSelect
                        ...
                        placeholder={
                          form.district ? "Chọn phường / xã" : "Chọn quận trước"
                        }
              ...
              <div className="p-6 space-y-3">
                {addrLoading ? (
                  [1, 2].map((i) => (
                    <div
                      key={i}
                      className="animate-pulse border border-gray-100 rounded-2xl p-4 space-y-2"
                    >
                  ...
                  <div className="text-center py-14">
                    ...
                    <button
                      onClick={openAddForm}
                      className="mt-4 px-5 py-2.5 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-xl transition-colors"
                    >
                      Thêm địa chỉ ngay
                    </button>
                  </div>
                  ...
                  addresses.map((addr) => (
                    ...
                    >
                      ...
                      <div className="flex items-start gap-3">
                        <div
                          className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${addr.isDefault ? "bg-red-100" : "bg-gray-100"}`}
                        >
                          <Home
                            className={`w-4 h-4 ${addr.isDefault ? "text-red-500" : "text-gray-400"}`}
                          />
                        </div>
                      ...
                      <div className="flex items-center gap-2 mt-3 ml-12">
                        <button
                          ...
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 text-xs font-medium transition-colors"
                        >
                        ...
                        {!addr.isDefault && (
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 text-xs font-medium transition-colors"
                          >
                        ...
                        {addresses.length > 1 && (
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-200 text-red-400 hover:bg-red-50 hover:border-red-200 text-xs font-medium transition-colors disabled:opacity-50"
                          >
          ...
          {tab === "orders" && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="px-6 py-16 flex flex-col items-center text-center">
                ...
                <Link
                  href="/sanpham"
                  className="mt-5 px-5 py-2.5 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-xl transition-colors"
                >
                  Mua sắm ngay
                </Link>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={pwSaving}
                  className="w-full bg-red-500 hover:bg-red-600 disabled:opacity-60 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors mt-2"
                >
