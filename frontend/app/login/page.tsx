...
export default function DangNhapPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // SỬA LỖI TẠI ĐÂY: Gộp và kiểm tra thông tin tên hiển thị chính xác tránh ghi đè lỗi dữ liệu
      const username =
        data.user?.hoTen ||
        data.user?.ten ||
        data.user?.username ||
      ...
      localStorage.setItem("smarthub_username", username);

      // Điều hướng về trang chủ sau khi đăng nhập thành công
      router.push("/");
      router.refresh(); // Làm mới trạng thái để nhận token mới lập tức trên các component khác
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-8 text-center relative">
        ...
        <p className="text-gray-500 text-sm mb-6">
          Vui lòng đăng nhập tài khoản Smarthub để xem
          <br />
