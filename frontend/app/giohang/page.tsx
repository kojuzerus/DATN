...
// Lấy token — hỗ trợ cả 2 key cũ và mới
function getToken() {
  return localStorage.getItem('token') || localStorage.getItem('smarthub_token') || null;
}
...
export default function GioHangPage() {
  ...
  const clearCart = async () => {
    if (!token || !confirm('Bạn có chắc muốn xóa toàn bộ giỏ hàng?')) return;
    try {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {items.map(item => (
                <div key={item._id}
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-gray-800 text-sm line-clamp-2 mb-1">{item.tenSanPham}</h3>
                    {item.variant && <p className="text-xs text-gray-400 mb-2">Phân loại: {item.variant}</p>}
                    <p className="text-red-500 font-bold text-base">{formatPrice(item.gia)}</p>
                  </div>

                  <div className="flex flex-col items-end justify-between shrink-0">
                    <button onClick={() => removeItem(item._id)} disabled={updating === item._id}
                    ...
                    <div className="flex items-center gap-2 bg-gray-50 rounded-xl border border-gray-200">
                      <button onClick={() => updateQty(item._id, item.soLuong - 1)}
                        disabled={updating === item._id || item.soLuong <= 1}
                        className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-red-500 disabled:opacity-30 transition font-bold text-lg">−</button>
                      <span className="w-8 text-center text-sm font-semibold text-gray-800">{item.soLuong}</span>
                      <button onClick={() => updateQty(item._id, item.soLuong + 1)}
                        disabled={updating === item._id}
                        className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-red-500 transition font-bold text-lg">+</button>
                    </div>

                    <p className="text-xs font-medium text-gray-500">{formatPrice(item.gia * item.soLuong)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 sticky top-24">
                ...
                <button onClick={() => router.push('/thanhtoan')}
